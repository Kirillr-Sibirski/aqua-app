...
| `lib.ts` | Receipt decoding, assertions, formatting, the story state file |
...
gitignored: they are generated by `make story-setup` in about a second.
...
| `npm test` in `web/` fails with `replacement transaction underpriced` | three `*.fork.test.ts` files ship from anvil #1 in parallel vitest workers, so their nonces collide. Only happens while a fork is up | `npx vitest run --no-file-parallelism` (271 passed, 11 skipped) |
