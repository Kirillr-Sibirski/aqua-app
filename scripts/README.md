# `scripts/` — local Base-fork demo infrastructure
...
make bootstrap                 # terminal 2: forge build + deploy router + fund wallets + write manifests
                               # (for the demo, use `make story-setup` instead: it runs this and freezes the state)
...
| `fork/bootstrap.ts` | Deploys the router from a Foundry artifact (`ROUTER_ARTIFACT`, default `contracts/out/StrikelineRouter.sol/StrikelineRouter.json`; constructor params matched by name, so both the 3-arg and the 5-arg `(aqua,weth,owner,name,version)` shapes work). Refuses to write a manifest unless the deployed address answers `tauNow(uint40)`, so a router without `RmmSwap`/`Coverage` cannot be recorded as the app's. Deploys with anvil account #0, funds accounts #0–#3 + `DEMO_ADDRESS` via whale impersonation (Aave aTokens / Morpho) with an `anvil_setStorageAt` balance-slot fallback, writes `fork/deployments.local.json` and `web/public/deployments/local.json`. Idempotent: reuses the router and tops balances up. |
...
make story-load                # evm_revert (or anvil_loadState after a restart) back to take one, about a second
...
`{ chainId, rpcUrl, blockNumber (fork pin), bootstrapBlock, forkChainId, aqua, officialRouter, router, routerName,
routerEip712, routerArtifact, routerOwner, weth, usdc, cbBtc,
